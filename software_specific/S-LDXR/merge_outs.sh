...
COLS_TO_KEEP=$(awk -F' ' -v remove="$REMOVE_COLS" '
    $2 !~ remove {printf "%s%s", (NR==1?"":","), $1}
' temp_cols.txt)
...
# Combine binned and continuous files (continuous file already has correct columns)
cat temp_binned.txt "$CONT_FILE" | awk 'NR==1 || !/^P1/' > "$OUTPUT_FILE"
