...
# create the new annot mat
    ...
    for j in range(len(names)):
        ...
        try:
            cut = pd.qcut(annot_vec, nbins, retbins=True, duplicates='drop')[1]
            if len(cut) - 1 < nbins:
                print(f"[WARN] Annotation '{nm}' produced only {len(cut) - 1} bins (expected {nbins}). Skipping.")
                continue
        except ValueError as e:
            print(f"[WARN] Skipping annotation '{nm}' due to qcut binning error: {e}")
            continue
